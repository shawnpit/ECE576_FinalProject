...
class Root(Tk):
    def __init__(self):
        ...
        self.title("Huffman Code Encoder/Decoder")
        self.minsize(640, 200)
 
        self.labelFrame = ttk.LabelFrame(self, text = "Encoder")
        self.labelFrame.grid(column = 0, row = 1, padx = 20, pady = 20)
        
        self.labelFrame2 = ttk.LabelFrame(self, text = "Decoder")
    ...
    def button(self):
        self.button = ttk.Button(self.labelFrame, text = "Browse a file to Encode",command = self.fileDialog)
        self.button.grid(column = 1, row = 1)
   
    def button2(self):
        self.button2 = ttk.Button(self.labelFrame2, text = "Browse a file to Decode",command = self.fileDialog2)
    ...
    def fileDialog(self):
        ...
        file = open(path.split('.')[0]+'_HE.bin', "wb")
        ...
        self.label2.configure(text = 'Encode file: '+path.split('.')[0]+'_HE.bin')
        t2=time.time()
        
        fileSize=os.stat(path.split('.')[0]+'_HE.bin').st_size
        ...
        file.write('Symbol\t\t\tFrequency\t\tBinary Value\n')
        for x in range (len(FreqlistSort)):
